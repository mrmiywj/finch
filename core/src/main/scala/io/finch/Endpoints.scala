...
trait Endpoints {
  ...
  private[finch] class Matcher(s: String) extends Endpoint[HNil] {
    override val run: Endpoint.State[HNil] = StateT(input =>
  ...
  private[finch] case class Extractor[A](name: String, f: String => Option[A]) extends Endpoint[A] {
    override val run: Endpoint.State[A] = StateT(input =>
      for {
        ss <- input.headOption
        aa <- f(ss)
  ...
  private[finch] case class StringExtractor(name: String) extends Endpoint[String] {
    override val run: Endpoint.State[String] = StateT(input =>
  ...
  private[finch] case class TailExtractor[A](
    ...
    override val run: Endpoint.State[Seq[A]] = StateT(input =>
   ...
   */
  object / extends Endpoint[HNil] {
    override val run: Endpoint.State[HNil] = StateT.pure(hnilFutureOutput)
    override def toString: String = ""
  }
